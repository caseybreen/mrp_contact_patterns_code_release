...
This notebook contains code to plot the post-stratified estimates of contact from our MRP model. 
...
predictions <- list.files(path = here('data', 'mrp', 'predictions_1000'), pattern = ".rds", full.names = T) %>%    
  map_dfr(read_rds)

## read in acs data
acs_state_symmetry <- readRDS(here("data", "mrp", "input", "acs_state_symmetry.rds")) %>% 
  group_by(state) %>% 
...
## baseline eigenvalue ratio 
prepandemic_eigenvalue <- 9.8547248

## baseline eigenvalue ratio 
baseline_eigenvalue <- summary_statistics %>%
  filter(month == as_date("2020-04-01")) %>%
  summarize(mean(leading_eigenvalue)) %>% 
  as.numeric()
...
ggsave(plot = state_level_mixing_matrices_symmetrical, height = 10, width = 13, file = here("code", "models", "mrp", "figures", glue("state_level_mixing_matrices_symmetrical{month_i}.png")))
...
ggsave(plot = raw_vs_modeled_plot, height = 12, width = 14, file = here("code", "models", "mrp", "figures", "raw_vs_modeled_plot.png"))
```
...
ggsave(plot = total_contacts_plot, height = 12, width = 18, file = here("code", "models", "mrp", "figures", "total_contact_state_plot.png"))
```
...
ggsave(plot = eigenvalue_plot_ribbon_month, height = 12, width = 18, file = here("code", "models", "mrp", "figures", "eigenvalue_ratio_state_plot.png"))
```
...
ggsave(plot = R0_plot_ribbon_month, height = 12, width = 18, file = here("code", "models", "mrp", "figures", "R0_state_plot.png"))
```
...
q_index_association_R0 <- q0_data %>%
  ...
  theme_cowplot() +
  labs(
    title = "Association between R0 and q_index (State Average)",
    y = "R0"
  )

text <- q0_data %>%
  filter(state %in% c("NY", "CA", "KY", "AL", "MN")) %>%
...
state_change_over_time <- q0_data %>%
  filter(state %in% c("NY", "CA", "KY", "AL", "MN")) %>%
  group_by(month, state) %>%
  summarize(
    q0 = mean(q0),
  ...
  arrange(month) %>%
  ggplot(aes(x = q0, y = R0, col = state)) +
  geom_point() +
  geom_path(size = 1.5) +
  geom_label_repel(data = text, aes(label = state)) +
  theme_cowplot() +
  ggsci::scale_color_lancet() +
  theme(legend.position = "none") +
  labs(
    x = "q-index",
    y = "R0",
    title = "State Trends over Time"
  )

q_index_plots <- cowplot::plot_grid(q_index_over_time, q_index_association_R0, state_change_over_time, labels = "auto")


ggsave(plot = q_index_plots, height = 12, width = 18, file = here("code", "models", "mrp", "figures", "q_index_plot.png"))
```
...
ggsave(plot = ca_plot, height = 18, width = 14, file = here("code", "models", "mrp", "figures", "ca_plot.png"))
```
...
left_eigenvector <-  summary_statistics %>% 
  ...
  mutate(left_eigenvector_standardized = abs(eigenvector / sum(eigenvector))) %>% 
  ungroup()

example_state <- left_eigenvector %>% 
  mutate(date = as_date("2020-04-11") + day_std) %>% 
  mutate(month = lubridate::floor_date(date, "month")) %>% 
  filter(month %in% c(as_date(" 2020-04-01"), as_date("2021-1-01"),  as_date("2021-5-01"))) %>%
  group_by(state, agecat, month) %>% 
  summarize(left_eigenvector_mean = mean(left_eigenvector_standardized),
            left_eigenvector_se = sd(left_eigenvector_standardized),
...
reproductive_values <- example_state %>% 
  ggplot(aes(x = agecat,
             y = left_eigenvector_mean,
             fill = state,
             ymin = lower,
             ymax = upper)) + 
  geom_col(position = "dodge", width = 1, size = 3) + 
...
ggsave(plot = reproductive_values, height = 7, width = 17, file = here("code", "models", "mrp", "figures", "reproductive_values_plot.png"))
```

```{r}
## right_eigenvector 
right_eigenvector <- summary_statistics %>% 
  filter(state %in% c("CA", "NY", "TX")) %>% 
  select(state, day_std, .draw, right_eigenvector) %>% 
  unnest(right_eigenvector) %>% 
  group_by(state, day_std, .draw) %>% 
  mutate(right_eigenvector_standardized = abs(eigenvector / sum(eigenvector))) %>% 
  ungroup()

## example state 
example_state_right <- right_eigenvector %>% x
  mutate(date = as_date("2020-04-11") + day_std) %>% 
  mutate(month = lubridate::floor_date(date, "month")) %>% 
  filter(month %in% c(as_date(" 2020-04-01"), as_date("2021-1-01"),  as_date("2021-5-01"))) %>%
  group_by(state, agecat, month) %>% 
  summarize(left_eigenvector_mean = mean(right_eigenvector_standardized),
            left_eigenvector_se = sd(right_eigenvector_standardized),
            lower = quantile(right_eigenvector_standardized, 0.1),
            upper = quantile(right_eigenvector_standardized, 0.9)) %>% 
  mutate(month_label = lubridate::month(month, label = TRUE, abbr = TRUE))

## plot 
equilibrium_number <- example_state_right %>% 
  ggplot(aes(x = agecat,
             y = left_eigenvector_mean,
             fill = state,
             ymin = lower,
...
ggsave(plot = equilibrium_number, height = 7, width = 17, file = here("code", "models", "mrp", "figures", "equilibrium_values_plot.png"))
```
...
```{r}
## Example states: California, April 2020
two_level_mixing_matrices_symmetrical <- contact_matrices_predictions %>%
    mutate(month = lubridate::format_ISO8601(month, precision = "ym")) %>% 
  filter(month == "2020-05") %>%
  filter(state %in% c("WA", "IA")) %>% 
 ...
 # viridis::scale_fill_viridis(name = str_wrap("Contacts per respondent", width = 10)) +
  ...
  labs(
    title = glue('State-Level Contact Matrices (May 2020)'),
    x = "Ego age",
    y = "Alter age"
  ) 

ggsave(plot = two_level_mixing_matrices_symmetrical, height = 6, width = 10, file = here("code", "models", "mrp", "figures", "two_state_eigenvalue_ratio_state_plot.png"))
...
eigenvalue_plot_ribbon_month_two_state <- monthly_estimates_eigenvalue %>%
  filter(state %in% c("CA", "UT")) %>% 
  mutate(state = case_when(
    state == "CA" ~ "California",
    state == "UT" ~ "Utah"
  )) %>% 
...
ggsave(plot = eigenvalue_plot_ribbon_month_two_state, height = 6, width = 10, file = here("code", "models", "mrp", "figures", "two_state_eigenvalue_ratio_state_plot.png"))
```
