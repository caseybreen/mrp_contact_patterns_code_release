---
title: "Calculate contact matrices and R0"
---
...
Specifically, the this code will: 

(1) Produce Post-stratified estimates 
(2) Adjust matrices for symmetry
(3) Estimate contact for youngest age group (not surveyed)
(4) Calculate contact matrices and estimates of R0, eigenvalue ratio, and total contact 
...
acs_state_reference <- readRDS(here("data", "mrp", "input", "acs_state_reference.rds")) %>% 
  filter(agecat != "[0,18)") %>% 
  droplevels() %>% 
...
acs_national_symmetry <- readRDS(here("data", "mrp", "input", "acs_national_symmetry.rds")) 

google_pca_data <- read_rds(file=here('data', 'mrp', 'input', 'google_mobility_pca.rds')) %>% 
  select(date, state = state, rolling_pca_3wks)

## Non-Household Model
nonhh_model <- read_rds(file = here('data', 'mrp', 'models', 'nonhh_model.rds'))

## Household Model 
hh_model <- read_rds(file = here('data', 'mrp', 'models', 'hh_model.rds'))
```
...
```{r}
make_predictions <- function(day, nonhh_model = nonhh_model, hh_model = hh_model, draws = 10){
  ...
  write_rds(x = newList, file = here('data', 'mrp', 'predictions_100', paste0('prediction_day', day, ".rds")))
...
# 
# ## create progress bar  
# pb <- progress_bar$new(total = length(days))

tic("total prediction time")
state_level_predictions_test <- map(days, ~make_predictions(.x, nonhh_model = nonhh_model, hh_model = hh_model, draws = 100))
toc()
```
