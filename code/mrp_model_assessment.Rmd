...
for (wave in waves){
  ...
  ## read in models 
  model_mobility <- read_rds(file = here('data', 'models', glue('nonhh_model_holdout_wave{wave}.rds')))
  model_baseline <- read_rds(file = here('data', 'models', glue('nonhh_model_no_mobility_holdout_wave{wave}.rds')))
  
  ## predictions for mobility data 
  predicted_draws_mobility <- add_predicted_draws(newdata = test_wave, object = model_mobility, ndraws = 500, allow_new_levels = T) %>%
  ...
  predicted_draws_baseline <- add_predicted_draws(newdata = test_wave, object = model_baseline, ndraws = 500, allow_new_levels = T) %>%
    mutate(numcc_predict = .prediction) %>% 
    ungroup() %>% 
    select(rid, state, agecat, gender, hhsize, race, weekday, day_std, .draw, numcc_predict, .category)
  ...
  ## add mse to results 
  results[[wave]] <- mse_error 
  
  ## print
  cat("Finished with wave ", wave)
